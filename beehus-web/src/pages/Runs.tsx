...
export default function Runs() {
  ...
  const fetchRuns = useCallback(async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL || "http://localhost:8000"}/dashboard/recent-runs?limit=100`,
      );
      setRuns(res.data);
    } catch (error) {
  ...
  const openFileSelection = async (run: Run) => {
    try {
      ...
      const preferred =
        ...
        res.data.some((opt) => opt.filename === run.selected_filename)
          ? run.selected_filename
          : res.data[0]?.filename || "";
  ...
  const confirmFileSelection = async () => {
    ...
    try {
      ...
      if (status === "pending_sheet_selection") {
        ...
        showToast(
          "File selection saved. Click Reprocess to execute.",
          "success",
        );
      }
      await fetchRuns();
  ...
  const confirmSheetSelection = async () => {
    ...
    try {
      ...
      showToast(
        "Sheet selection saved. Click Reprocess to execute.",
        "success",
      );
      await fetchRuns();
    } catch (error) {
  ...
  const handleStop = async (runId: string) => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_API_URL || "http://localhost:8000"}/runs/${runId}/stop`,
      );
      ...
      setRuns(
        runs.map((run) =>
          run.run_id === runId ? { ...run, status: "failed" } : run,
        ),
      );
    } catch (error) {
  ...
  const handleReprocess = async (run: Run) => {
    ...
    try {
      ...
      if (status === "pending_file_selection") {
        ...
        await openSheetSelection(
          run,
          res.data?.selected_filename || run.selected_filename || undefined,
        );
      } else {
      ...
      <div className="p-8 max-w-7xl mx-auto space-y-8">
        <header className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <span
              className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-medium ${wsConnected ? "bg-green-500/20 text-green-300 border-green-500/30" : "bg-red-500/20 text-red-300 border-red-500/30"}`}
            >
              <span
                className={`h-2 w-2 rounded-full ${wsConnected ? "bg-green-400" : "bg-red-400"}`}
              ></span>
              {wsConnecting
                ? "WS connecting…"
                : wsConnected
                ...
                runs.map((run) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-white">
                      {run.job_name || run.connector}
                    </td>
                    <td className="px-6 py-4">
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusBadge(run.status)}`}
                      >
                        {run.status === "running" && (
                          <span className="w-1.5 h-1.5 bg-brand-400 rounded-full mr-1.5 animate-pulse"></span>
                        )}
                    ...
                    <td className="px-6 py-4">
                      <div className="space-y-1">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getProcessingBadge(run.processing_status)}`}
                        >
                          {(run.processing_status || "not_required").replaceAll(
                            "_",
                            " ",
                          )}
                        </span>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="glass rounded-xl border border-white/10 p-6 w-full max-w-lg space-y-4">
            ...
            <p className="text-sm text-slate-400">
              Run #{fileModalRun.run_id.slice(0, 8)}
            </p>
            <select
              value={selectedFile}
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                disabled={
                  !selectedFile || processingRunId === fileModalRun.run_id
                }
                className="px-4 py-2 rounded bg-brand-600 text-white text-sm disabled:opacity-60"
              >
                {processingRunId === fileModalRun.run_id
                  ? "Saving..."
                  : "Confirm"}
              </button>
            </div>
          ...
          <div className="glass rounded-xl border border-white/10 p-6 w-full max-w-lg space-y-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                disabled={
                  !selectedSheet || processingRunId === sheetModalRun.run_id
                }
                className="px-4 py-2 rounded bg-brand-600 text-white text-sm disabled:opacity-60"
              >
                {processingRunId === sheetModalRun.run_id
                  ? "Processing..."
                  : "Confirm"}
              </button>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Run #
                  {logModalRun.id?.slice?.(0, 8) ||
                    logModalRun.run_id.slice(0, 8)}
                </p>
              </div>
              <button
